...
# Brave Search MCP 配置脚本
# 此脚本将添加 Brave Search MCP 服务器配置到 Cursor 的 MCP 配置文件
...
# 检查是否已经配置了 brave-search MCP
if grep -q '"brave-search"' "$CURSOR_MCP_CONFIG"; then
    echo "警告: 配置文件中已存在 'brave-search' MCP 服务器配置"
...
echo ""
echo "========================================="
echo "Brave Search MCP 配置"
echo "========================================="
echo ""
echo "请获取您的 Brave Search API Key:"
echo "1. 访问 https://api.search.brave.com/"
echo "2. 注册或登录账户"
echo "3. 在 API 密钥管理页面生成新的 API 密钥"
echo ""
read -p "请输入您的 Brave Search API Key (留空将使用占位符): " BRAVE_API_KEY
...
# 这里使用 Python，因为大多数 macOS 系统都预装了 Python
...
try:
    ...
    print("✓ 已成功添加 Brave Search MCP 配置")
    sys.exit(0)
...
EOF
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ "$BRAVE_API_KEY" = "YOUR_BRAVE_API_KEY_HERE" ]; then
            echo ""
            echo "⚠️  重要: 请手动编辑配置文件并替换 YOUR_BRAVE_API_KEY_HERE 为您的实际 API Key"
        fi
        echo ""
